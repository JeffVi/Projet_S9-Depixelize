...
void similarity_graph::resolu()
{
	...
	for(int i=0;i<rows-1;i++)
	{
		...
		{
			if(pixels_nodes.at<int>(i,j+1,4) == 1 && pixels_nodes.at<int>(i,j+1,6) == 1 && pixels_nodes.at<int>(i+1,j,0) == 1 && pixels_nodes.at<int>(i+1,j,2) == 1)
			{
				pixels_nodes.at<int>(i,j,3) = 0;
				pixels_nodes.at<int>(i,j+1,5) = 0;
				pixels_nodes.at<int>(i+1,j,1) = 0;
				pixels_nodes.at<int>(i+1,j+1,7) = 0;
			}
		}
	}
	...
	for(int i=0;i<rows-1;i++)
	{
		for(int j=0;j<cols-1;j++)
		{
			if(pixels_nodes.at<int>(i,j,3) == 1 && pixels_nodes.at<int>(i,j+1,5) == 1)
			{
				int cpt1 = 0;
				int cpt2 = 0;
				for(int k=0;k<8;k++)
				{
					cpt1 += pixels_nodes.at<int>(i,j,k);
					cpt2 += pixels_nodes.at<int>(i,j+1,k);
				}
				if(cpt1>cpt2)
				{
					pixels_nodes.at<int>(i,j,3) = 0;
